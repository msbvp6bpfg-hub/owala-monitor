...
TARGET_LIST = [
    ...
    {"store": "誠品 (不鏽鋼-新款式)", "url": "https://www.eslite.com/product/10052271402683051105009", "checker": check_eslite_html},

    # 4. 誠品 - FreeSip Tritan 透明款
...
def main():
    # 每次啟動在雲端持續運行 25 分鐘，每 5 分鐘精確檢查一次
    ...
    while (time.time() - start_time) < (total_duration_minutes * 60):
        scan_once()
        print(f"⏳ 等待 {interval_minutes} 分鐘後進行下一次檢查...\n")
